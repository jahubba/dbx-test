...
```
dbx execute --cluster-name=<name of interactive cluster> --job=pg_demo-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=pg_demo-sample-integration-test
```
